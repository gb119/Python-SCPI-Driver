...
class Param(object):
    ...
    def format_write(self,tree,value):
        ...
        if self.write is None:
            raise CommandError("Read only parameter {} trying to be written with {}".format(tree,value))
        if isinstance(write,str):
            ...
            if not isinstance(value,Iterable):
                raise ValueError("{} expects an iterable value not a {}".format(tree,type(value)))
            value=np.array(value).astype(self.write.dtype)
            length=self.write.size
            ret=[]
            for ix in range(value.size//length+1):
                ret.append("{} {}".format(tree,",".join(value[ix*length:(ix+1)*length].astype(str))))
            return "\n".join(ret)
        else:
            return ("{} {}".format(tree,value))
...
class SCPI_Instrument_Mixin(object):
    ...
    def _get_path(self,name):
        ...
        for part in tree.split(":"):
            if part not in cmd_dict:
                raise AttributeError("{} not recognised by driver as a SCPI command!".format(tree))
            else:
    ...
    def __getattr__(self,name):
        ...
        if isinstance(cmd_dict,Mapping):# Sub path returned so we're constructing an instance of ourselves from here.
...
class _proxy(object):
    ...
    def __setattr__(self,name,value):
        ...
        cmd_dict,tree,full_path=self._get_path(name)
        if not isinstance(cmd_dict,Param):
            raise CommandError("Non terminal SCPI command path {} trying to be set a value of {}!".format(tree,value))
